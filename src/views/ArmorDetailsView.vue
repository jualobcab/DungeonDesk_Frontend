    ...
    >
      <div class="bg-gray-900 border border-amber-900/40 rounded-lg p-8 max-w-sm w-full shadow-xl flex flex-col items-center">
        <p class="text-lg text-gray-100 mb-6 text-center">
          ¿Seguro que quieres borrar la armadura <span class="text-amber-400 font-bold">{{ armor.name }}</span>?
        </p>
...
const editArmor = () => {
  router.push(`/equipment/${route.params.id}/edit`)
}

const confirmDelete = async () => {
  try {
    await adminService.deleteArmor(route.params.id)
    showDeleteModal.value = false
    router.push('/equipment/armors')
  } catch (error) {
