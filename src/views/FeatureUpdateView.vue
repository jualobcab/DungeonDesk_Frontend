...
const fetchAllClassesAndSubclasses = async () => {
  ...
  allSubclasses.value = subclassesRes
    ...
    .map(sub => ({
      ...sub,
      class_id: sub.class_id // asegúrate de que cada subclase tiene class_id
    }))
}
...
// Guardar cambios de nombre/desc
const validate = () => {
  errors.value = {}
...
const removeFeatureFromClass = async (cls) => {
  try {
    // No hay endpoint específico, así que puedes crear uno o reutilizar addFeatureToClass con level null
    await adminService.addFeatureToClass({
      class_id: cls.class_id,
      feature_id: feature.value.feature_id,
      level: null // El backend debe interpretar esto como "eliminar"
    })
    await fetchFeature()
  } catch (e) {
...
const removeFeatureFromSubclass = async (sub) => {
  try {
    // No hay endpoint específico, así que puedes crear uno o reutilizar addFeatureToSubclass con level null
    await adminService.addFeatureToSubclass({
      subclass_id: sub.subclass_id,
      feature_id: feature.value.feature_id,
      level: null // El backend debe interpretar esto como "eliminar"
